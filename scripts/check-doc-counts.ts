 ...
 * source-of-truth values. When a doc quotes the test count ("356 tests") and
 * the suite later grows, that number rots. This script makes the rot fail CI
 * loudly instead of slipping through to a published README.
 *
 * Audits:
 *
 *   1. Test-count drift. Runs `bun test`, parses its `Ran N tests` summary as
 *      the source of truth, then scans README.md (and CLAUDE.md when present)
 *      for any "N tests" / "N TypeScript tests" claim and requires every one
 *      to equal the real count. A doc with no claim is fine — only a claim
 *      that disagrees is a violation.
 *
...
/** Docs that may carry a test-count claim. CLAUDE.md is scanned only when it
 *  exists (feats ships a README but no CLAUDE.md today). */
const TEST_COUNT_DOC_CANDIDATES: readonly string[] = ["README.md", "CLAUDE.md"];
...
Audits:
  1. Test-count drift — README.md (+ CLAUDE.md if present) "N tests" claims vs
     the live 'Ran N tests' count from \`bun test\`.
 ...
 *  lines never feed the TypeScript audit. */
...
function main(): number {
  ...
  const testDocs: Doc[] = [];
  for (const path of TEST_COUNT_DOC_CANDIDATES) {
    // CLAUDE.md is optional; skip silently when absent.
