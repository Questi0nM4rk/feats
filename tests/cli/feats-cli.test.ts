...
describe("runFeatsCli — meta flags", () => {
  ...
  test("-h also prints usage and exits 0", async () => {
    const stdoutChunks: string[] = [];
    const realOut = process.stdout.write.bind(process.stdout);
    process.stdout.write = ((c: string | Uint8Array) => {
      stdoutChunks.push(typeof c === "string" ? c : new TextDecoder().decode(c));
    ...
    try {
      const exit = await runFeatsCli(["-h"]);
      expect(exit).toBe(0);
      expect(stdoutChunks.join("")).toContain("Exit codes:");
    } finally {
      process.stdout.write = realOut;
    }
  });
  ...
  test("--version prints version string and exits 0", async () => {
    ...
    try {
      const exit = await runFeatsCli(["--version"]);
      expect(exit).toBe(0);
      // Should print "feats X.Y.Z"
      expect(stdoutChunks.join("")).toMatch(/^feats \d+\.\d+\.\d+/);
    } finally {
      process.stdout.write = realOut;
    }
  });

  test("-v also prints version string and exits 0", async () => {
    const stdoutChunks: string[] = [];
    const realOut = process.stdout.write.bind(process.stdout);
    process.stdout.write = ((c: string | Uint8Array) => {
      stdoutChunks.push(typeof c === "string" ? c : new TextDecoder().decode(c));
    ...
    try {
      const exit = await runFeatsCli(["-v"]);
      expect(exit).toBe(0);
      expect(stdoutChunks.join("")).toMatch(/feats \d+\.\d+\.\d+/);
    } finally {
      process.stdout.write = realOut;
...
describe("parseArgs — additional edge cases", () => {
  ...
  test("multiple --require flags accumulate into requires array", () => {
    const a = parseArgs(["--require", "steps/a.ts", "--require", "steps/b.ts"]);
    expect(a.requires).toEqual(["steps/a.ts", "steps/b.ts"]);
  });

  test("multiple --reporter flags accumulate into reporters array", () => {
    const a = parseArgs(["--reporter", "pretty", "--reporter", "junit:out.xml", "--reporter", "cucumber-json:out.json"]);
    expect(a.reporters).toEqual(["pretty", "junit:out.xml", "cucumber-json:out.json"]);
  });

  test("--require at the end of argv throws CliError (missing value)", () => {
    expect(() => parseArgs(["--require"])).toThrow(/requires a value/);
  });

  test("--reporter at the end of argv throws CliError (missing value)", () => {
    expect(() => parseArgs(["--reporter"])).toThrow(/requires a value/);
  });

  test("flags and positionals can be interleaved", () => {
    const a = parseArgs(["a.feature", "--tags", "@smoke", "b.feature"]);
    expect(a.features).toEqual(["a.feature", "b.feature"]);
    expect(a.tags).toBe("@smoke");
  });

  test("CliError has exitCode=2", () => {
    let caught: unknown;
    try {
    ...
    expect((caught as any).exitCode).toBe(2);
  });
});

describe("runFeatsCli — no feature files matched", () => {
  test("when feature glob matches nothing → exit 2 with message", async () => {
    const dir = await mkdtemp(join(tmpdir(), "feats-cli-no-features-"));
    // Write a steps file but NO .feature files
    await writeFile(
      join(dir, "test.steps.ts"),
      `import { Given } from "${FEATS_ROOT}/src/feats.ts";\nGiven("x", () => {});\n`,
      "utf-8",
    );
    ...
    try {
      const exit = await runFeatsCli(
        ["nonexistent.feature", "--require", "test.steps.ts"],
        dir,
      );
      expect(exit).toBe(2);
      expect(stderrChunks.join("")).toContain("no feature files matched");
    } finally {
      process.stderr.write = realErr;
    }
  });
...
describe("runFeatsCli — tag filtering via --tags", () => {
  test("--tags filters out non-matching scenarios, passing run exits 0", async () => {
    const { exitCode, stdout } = await runWithProject({
      feature: `Feature: Tagged\n  @smoke\n  Scenario: smoke run\n    Given smoke step\n  @slow\n  Scenario: slow run\n    Given slow step\n`,
      steps: `
import { Given } from "${FEATS_ROOT}/src/feats.ts";
Given("smoke step", () => {});
Given("slow step", () => { throw new Error("should not run"); });
`,
      argv: ["--reporter", "pretty", "--tags", "@smoke"],
    });
    expect(exitCode).toBe(0);
    expect(stdout).toContain("smoke run");
  });
});
