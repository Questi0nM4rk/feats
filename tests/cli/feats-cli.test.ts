...
describe("runFeatsCli — meta flags", () => {
  ...
  test("--version / -v prints 'feats <version>' and exits 0", async () => {
    for (const flag of ["--version", "-v"]) {
      ...
      try {
        const exit = await runFeatsCli([flag]);
        expect(exit).toBe(0);
        const out = stdoutChunks.join("");
        // Must start with "feats " followed by a semver-like version.
        expect(out).toMatch(/^feats \d+\.\d+\.\d+/);
      } finally {
        process.stdout.write = realOut;
      }
    }
...
describe("parseArgs — additional edge cases", () => {
  test("--require with no following value throws CliError", () => {
    expect(() => parseArgs(["--require"])).toThrow(/requires a value/);
  });

  test("--reporter with no following value throws CliError", () => {
    expect(() => parseArgs(["--reporter"])).toThrow(/requires a value/);
  });

  test("multiple --require flags accumulate into requires array", () => {
    const a = parseArgs(["--require", "a.steps.ts", "--require", "b.steps.ts"]);
    expect(a.requires).toEqual(["a.steps.ts", "b.steps.ts"]);
  });

  test("--version / -v set version flag", () => {
    expect(parseArgs(["--version"]).version).toBe(true);
    expect(parseArgs(["-v"]).version).toBe(true);
  });

  test("mixed positionals and flags are parsed correctly", () => {
    const a = parseArgs(["feat.feature", "--tags", "@smoke", "other.feature"]);
    expect(a.features).toEqual(["feat.feature", "other.feature"]);
    expect(a.tags).toBe("@smoke");
  });
});

describe("runFeatsCli — no feature files matched", () => {
  test("exits 2 when feature glob matches no files", async () => {
    const dir = await mkdtemp(join(tmpdir(), "feats-cli-no-feat-"));
    // Write a steps file but no feature file.
    await writeFile(join(dir, "test.steps.ts"), `// empty\n`, "utf-8");
...
describe("runFeatsCli — tag filtering", () => {
  test("--tags filters out non-matching scenarios", async () => {
    const { exitCode, stdout } = await runWithProject({
...
Given("a slow step", () => {});
`,
      argv: ["--tags", "@smoke", "--reporter", "pretty"],
    });
