...
describe("generateSyntheticFeature", () => {
  test("returns the expected number of scenarios for plain-scenario requests", () => {
    ...
    // 5 plain scenarios + 10 outline examples
  ...
  test("has a background with a single step", () => {
    const feature = generateSyntheticFeature(1);
  ...
  test("scenarioCount=0 produces only the 10 outline examples", () => {
    ...
    // All should be outline examples (name contains the outline title or values)
  ...
  test("outline examples carry substituted step text", () => {
    const feature = generateSyntheticFeature(0);
